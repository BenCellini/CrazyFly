...
%      	ratio       :   frame ratio to compare top & bottom of fly (default = 1/4)
%
%   OUTPUT:
...
if debug
    ...
    fig.Position(1:4) = [2 2 10 5];
        ...
        ax(2) = subplot(1,2,2); cla ; hold on; axis image
            title({'Click "flip" if fly head is pointing down in right image.', ...
	...
	uicontrol('Style', 'pushbutton', 'String', 'Flip', ...
              'Callback', @flipcheck, 'Position', [75 50 60 20])

 	uicontrol('Style', 'pushbutton', 'String', 'Continue', ...
              'Callback', @skipcheck, 'Position', [500 50 60 20])
   	waitfor(fig)
else
    fig = [];
    ax  = [];
