function [angle, m, pts, k, c_mean, c_mean_all] = tracktip(img, mask, rot, norm, npts, mode, arg, dthresh, rmv_out)
%% tracktip: track a tip in an image
%
%   INPUT:
...
%       arg    	    :   percentiles for median or # of clusters
...
%       pts   	    : point locations
%       k           : cluster groups
%       c_mean      : mean position of each cluster
%       c_mean_all  : mean position of all clusters
%
    ...
    if nargin < 8
        ...
        if nargin < 7 || isempty(arg)
            ...
            switch mode
                ...
                case 'clust' % average of cluster centroids
    ...
    switch mode
        ...
        case 'clust' % agglomerative clustering to find tip(s)
            if arg == 1 % no clustering to do if there's only 1 group
                ...
                % Get mean xy of each cluster
                c_mean = cell(arg, 1);
                for c = 1:arg
                    c_mean{c} = mean(pts(k==c, :), 1);
                end

                % Mean of all clusters
                c_mean_all = mean(cat(1, c_mean{:}), 1);
